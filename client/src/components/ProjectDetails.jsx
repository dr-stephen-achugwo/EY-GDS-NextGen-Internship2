...
const ProjectDetails = ({ project }) => {
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SERVER_URL}/api/projects/${project._id}`,
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <div
      className={`m-5 p-5 rounded-xl border border-slate-700 flex flex-col gap-5 w-[30rem] ${
    ...
    >
      <div className="project-top">
        ...
        <span className="uppercase text-xs tracking-widest text-slate-400 font-medium">
          {project.tech}
        </span>
      </div>

      <div className="project-mid flex gap-10">
      ...
      <div className="project-bottom flex gap-5">
        <button
          ...
          className="bg-sky-400 text-slate-900 py-2 px-5 rounded shadow-xl hover:bg-sky-50 duration-300 cursor-pointer"
        >
          Update
        </button>
        <button
